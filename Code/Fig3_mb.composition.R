#### Rush CX study: Fig. 3, sample composition and structure
##### Anna M. Seekatz
##### 12.18.17

###### Figures created:
	- Fig. 3A,B : PCOA and biplot
	- Fig. 3C: heatmap of avg abundance of top OTUs per PAM cluster
	- Fig. S2: heatmap of abundance of top OTUs in each individual samples


####### Files used:
	- rushfinalCX_summary.w.pam.txt
	- rushfinal.CXsamples.0.03.0.03.filter.spearman.corr.axes
	- rushfinal.03_taxnames_allOTUs.txt
	- rushfinalCX_summary.w.pam.txt
...
plot1<-plot(data$pcoa03_axis1, data$pcoa03_axis2, ylab="PCoA 2 (6.8%)", xlab="PCoA 1 (8.3%)", data = data, pch=21, col="black", bg=group.col(data$cre_status), cex=1, cex.lab=1, cex.axis=1, ylim=c(-0.6, 0.6), xlim=c(-0.6, 0.6))
...
Arrows(0, 0, x1=-0.220818, y1=0.199955, lty=1, arr.type="triangle") #otu3
...
text(-0.220818, 0.199955, label="OTU003 \nEnterobacteriaceae", cex=.5, pos=1)
...
legend("topleft", paste("Pam Cluster", as.character(unique(rushsum$phylopam3_cluster)), sep=" "), col=PamCol, pch=15, cex=0.8)



### Chi-square for testing KPC-Kp(-)/(+) in PAM clusters:

library(MASS)       

rushsum<-read.table(file='manuscript/Git/Data/rushfinalCX_summary.w.pam.txt', header=TRUE, sep="\t")
tbl = table(rushsum$cre_status, rushsum$phylopam3_cluster) 
tbl  
...
#data:  tbl
#X-squared = 4.5806, df = 2, p-value = 0.1012
